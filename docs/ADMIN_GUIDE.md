...
It is used **only when there is a single conflict**. Unlike
`created`/`updated`/`deleted`, its placeholders refer to the **existing**
reservation that caused the conflict, not the request that was rejected:
`{user}` is the existing reservation's owner, `{time}` is its period, and
`{resource}` is the specific resource that conflicted. It is shown to the
user who attempted the reservation, using the format/template settings of
the conflicting resource's notification config (not a resource-wide
broadcast).

`conflict_item` is used when **multiple conflicts occur simultaneously**,
to list each conflict in a compact format without a heading. When multiple
conflicts happen, the system automatically prepends a count line such as
"2 conflicting reservations" (this line cannot be customized; it is
generated by the builder, not from a template). Each conflict is then
rendered using the `conflict_item` template. If `conflict_item` is not
defined, a default compact format is used.
...
different existing reservations — the resulting messages are shown
together: multiple conflicts display a count line + `conflict_item` list,
single conflict displays using the `conflict` template. No conflict is
silently dropped.
