...
`conflict_item`は**複数の競合が発生した場合**に、各競合をリスト表示する際に
使われます。`conflict`と異なり、見出しを含まない簡潔な形式です。
複数競合時は最初に「N件の予約と重複しています。」という件数行が
ビルダー側で自動的に付加され、その後、各競合に対して`conflict_item`
テンプレートを適用した項目が表示されます。件数行はテンプレートで
カスタマイズできません。`conflict_item`を定義しない場合は、以下の
デフォルト形式で表示されます。

1回の予約リクエストで複数リソース（例: サーバー全体）を指定し、
そのうち複数件が（場合によっては異なる既存予約と）競合した場合は、
複数の競合がある場合は件数行 + `conflict_item` リスト、
単一の競合の場合は `conflict` テンプレートで表示されるため、
一部の競合が黙って握りつぶされることはありません。
