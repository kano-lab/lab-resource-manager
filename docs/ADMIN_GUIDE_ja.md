...
- **OSユーザー名**: GPUサーバーをチェックボックスで複数選択し、その人物のOSユーザー名と
  メールアドレスを入力します。選択した全サーバーに同じユーザー名で一括リンクされるため、
  全サーバーで同じユーザー名を使っている場合は1回の送信で済みます。サーバーごとに
  ユーザー名が異なる場合は、同じユーザー名を使うサーバーだけをグループにして複数回に
  分けて実行してください。これにより、上記の未予約利用の事後予約提案・無断使用DM機能が
  「実際に誰がプロセスを実行しているか」を特定できるようになります。これを行わないと、
  観測されたOSユーザー名は誰にも解決できません。
